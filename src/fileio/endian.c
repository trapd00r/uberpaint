 ...
 * endian
 *
 *  read and write routines for little endian files
 */
...
char endian_little_read_8 (FILE * fp)
{
    return (getc(fp));
}

// read a 16 bit word/uword
int endian_little_read_16(FILE * fp)
{
    int a,b,z;
    a = endian_little_read_8(fp) & 0x00ff;
    b = endian_little_read_8(fp) & 0x00ff;
    z = (a<<8)+(b);
    return(z);
}

// read a 32bit long/ulong
long endian_little_read_32(FILE * fp)
{
    int a,b;
    long z;
    a = endian_little_read_16(fp) & 0x00ffff;
    b = endian_little_read_16(fp) & 0x00ffff;
    z = (a<<16)+(b);
    return(z);
}
