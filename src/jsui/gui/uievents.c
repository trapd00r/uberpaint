...
)
{
	...
	if (event->button.button == SDL_BUTTON_WHEELUP) {
	    ...
	    if (jsr) jsr->mb = buttonstates;
	    if (jsr) ret = jsui_dialog_tell_message(
			jsr->dialog, jsr->focus_obj, JSUI_MSG_USCROLL); 
            else DO_FALLBACK(JSUI_MSG_USCROLL, x, y, buttonstates);
	} else
	if (event->button.button == SDL_BUTTON_WHEELDOWN) {
	    ...
	    if (jsr) jsr->mb = buttonstates;
	    if (jsr) ret = jsui_dialog_tell_message(
			jsr->dialog, jsr->focus_obj, JSUI_MSG_DSCROLL); 
            else DO_FALLBACK(JSUI_MSG_DSCROLL, x, y, buttonstates);
	}
